...
namespace M220N.Repositories
{
    public class CommentsRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Builders<Comment>.Filter.Where(x => x.MovieId == movieId && x.Id == commentId && x.Name == user.Name),
            Builders<Comment>.Update.Set("Text", comment).Set("Date", DateTime.UtcNow),
            ...
            User user, CancellationToken cancellationToken = default)
        {
            ...
            _commentsCollection.DeleteOne(
                Builders<Comment>.Filter.Where(
                    c => c.MovieId == movieId
                         && c.Id == commentId && user.Name == c.Name));
            ...
            */
            try
            {
                ...
                result = await _commentsCollection
                  ...
                  .Aggregate()
                  .Group(x => x.Name, c => new { Count = c.Count(), Id = c.Key })
                  .SortByDescending(g => g.Count).Limit(20).Project(x => new ReportProjection
                  {
                      Count = x.Count,
                      Id = x.Id,
                  }).ToListAsync();
