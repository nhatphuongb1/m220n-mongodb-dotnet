...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb://phuong:phuong123@ac-r9hnnbm-shard-00-00.asjzgv4.mongodb.net:27017,ac-r9hnnbm-shard-00-01.asjzgv4.mongodb.net:27017,ac-r9hnnbm-shard-00-02.asjzgv4.mongodb.net:27017/?ssl=true&replicaSet=atlas-10dbde-shard-0&authSource=admin&retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            {
                var listWrites = datePipelineResults.Select(item => new UpdateOneModel<Movie>(Builders<Movie>.Filter.Eq(m => m.Id, item.Id), Builders<Movie>.Update.Set(m => m.LastUpdated, DateTime.Parse(item.LastUpdated.ToString()))));
                BulkWriteResult<Movie> bulkWriteDatesResult =await _moviesCollection.BulkWriteAsync(listWrites); 
            ...
            {
                var rates = ratingPipelineResults.Select(item => new UpdateOneModel<Movie>(Builders<Movie>.Filter.Eq(m => m.Id, item.Id), Builders<Movie>.Update.Set(m =>m.Imdb.Rating, double.Parse(item.Imdb.Rating.ToString()))));
                BulkWriteResult<Movie> bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(rates);
                 // TODO Ticket: Call  _moviesCollection.BulkWriteAsync, passing in the
